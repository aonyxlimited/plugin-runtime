...
test("runtime/lifecycle-error-initialize", async () => {
	...
	await expect(runtime.initialize()).rejects.toBeInstanceOf(TestError);
...
test("runtime/lifecycle-error-start", async () => {
	...
	await expect(runtime.start()).rejects.toBeInstanceOf(TestError);
...
test("runtime/lifecycle-error-terminate", async () => {
	const initializedPluginNames: string[] = [];
	const terminatedPluginNames: string[] = [];
	const startedPluginNames: string[] = [];
	const stoppedPluginNames: string[] = [];
	...
	class InitPluginStopFailer extends InitPluginA {
		manifest = {
			name: "InitPluginC",
		};
		async terminate(): Promise<void> {
	...
	const pluginA = new InitPluginA();
	const pluginB = new InitPluginB();
	const pluginC = new InitPluginStopFailer();
	runtime.register(pluginA);
	runtime.register(pluginB);
	runtime.register(pluginC);
	...
	await expect(runtime.terminate()).rejects.toBeInstanceOf(AggregateError);
...
test("runtime/lifecycle-error-stop", async () => {
	const initializedPluginNames: string[] = [];
	const terminatedPluginNames: string[] = [];
	const startedPluginNames: string[] = [];
	const stoppedPluginNames: string[] = [];
